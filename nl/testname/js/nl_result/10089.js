const results = {
  "1": {
    "title": "Ben jij de Diamant van het seizoen of het mikpunt van Lady Whistledown?",
    "resultTitle": "💎 De Diamant van het Seizoen",
    "description": `
Je straalt vanzelf.
Mensen kunnen niet
om je heen.

Als Diamant van
het Seizoen trek je
aandacht zonder moeite.
Je combineert charme
met zelfvertrouwen,
en zelfs roddels
lijken je alleen maar
interessanter te maken.

Anderen kijken naar je op —
soms met bewondering,
soms met jaloezie.
    `,
    "mainPic": "/testname/img/21_ef3df131800b4132a7f0c78bfc10f526.jpeg"
  },
  "2": {
    "title": "Ben jij de Diamant van het seizoen of het mikpunt van Lady Whistledown?",
    "resultTitle": "🖋 De Scherpe Observator",
    "description": `
Je ziet alles,
ook wat anderen missen.

Je bent slim,
strategisch
en altijd een stap vooruit.
Hoewel je niet altijd
in de spotlight staat,
heb je vaak meer invloed
dan zichtbaar is.

Lady Whistledown
zou je respecteren —
en misschien zelfs vrezen.
    `,
    "mainPic": "/testname/img/22_7ffa1bc5f9f040cda6cba1c1528cbd70.jpeg"
  },
  "3": {
    "title": "Ben jij de Diamant van het seizoen of het mikpunt van Lady Whistledown?",
    "resultTitle": "🌸 De Gevoelige Ziel",
    "description": `
Je hart is groter
dan de society.

Je voelt alles intens.
Roddels en verwachtingen
raken je diep,
maar dat maakt je ook
empathisch
en oprecht.

In een wereld
vol maskers
ben jij verfrissend echt.
    `,
    "mainPic": "/testname/img/23_daf6d2e69c334b7f842bc34472e47611.jpeg"
  },
  "4": {
    "title": "Ben jij de Diamant van het seizoen of het mikpunt van Lady Whistledown?",
    "resultTitle": "👑 Het Geslepen Doelwit",
    "description": `
Je bent ambitieus —
en dat blijft
niet onopgemerkt.

Je weet wat je wilt
en bent niet bang
om ervoor te gaan.
Dat maakt je krachtig,
maar ook een makkelijk
mikpunt.

Lady Whistledown
zou je naam vaker noemen
dan je lief is.
    `,
    "mainPic": "/testname/img/24_f2648cdb6a92416890dcc4d455eedbea.jpeg"
  }
};
