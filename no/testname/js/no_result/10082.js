const results = {
  "1": {
    "title": "Klarer du deg alene på fjellet? Test dine overlevelsesevner!",
    "resultTitle": "🐐 Flest B: Fjellgeita (Eksperten)",
    "description": `
Du er en drøm
å ha med på tur!

Du følger
Fjellvettreglene
til punkt og prikke,
har respekt for naturen
og vet nøyaktig
hva du skal gjøre
hvis krisen oppstår.

Du har kontroll
på utstyret,
tar smarte valg
og holder hodet kaldt.

Fjellet er
ditt andre hjem.
    `,
    "mainPic": "./testname/img/en_img/58_4cb0b37158ae45d08e41710d1c078716.png"
  },
  "2": {
    "title": "Klarer du deg alene på fjellet? Test dine overlevelsesevner!",
    "resultTitle": "🥾 Flest A eller C: Den tøffe turgåeren (Læregutten)",
    "description": `
Du har mye
turglede
og god fysikk,
men tar kanskje
litt for stor risiko.

Du stoler litt
for mye på flaks
og moderne teknologi.
Husk:
fjellet tar ikke hensyn
til ambisjonene dine.

Les deg opp
på kart og kompass,
så blir du helt trygg!
    `,
    "mainPic": "./testname/img/en_img/59_eb8446656a144ac69c859ea42883d196.jpeg"
  },
  "3": {
    "title": "Klarer du deg alene på fjellet? Test dine overlevelsesevner!",
    "resultTitle": "☕ Flest D: Søndagsturisten (Bymennesket)",
    "description": `
Du trives best
når stien er godt merket
og det er kort vei
til en vaffelstue.

Din styrke er at
du vet når du trenger hjelp,
men overlevelsesevnene dine
i villmarka er begrenset.

Hold deg til
populære stier
i godt vær,
eller gå med
en erfaren guide!
    `,
    "mainPic": "./testname/img/en_img/60_3a7964cbd6294346807b577cf8ec92f8.jpeg"
  },
  "4": {
    "title": "Klarer du deg alene på fjellet? Test dine overlevelsesevner!",
    "resultTitle": "🌲 Jevn blanding: Den forsiktige eventyreren",
    "description": `
Du har
grunnkunnskapene
på plass,
men blir usikker
når ting blir vanskelig.

Du tar ingen
unødvendige sjanser,
og det er bra,
men du mangler kanskje
erfaringen med å improvisere
i nødssituasjoner.

Fortsett å gå tur —
erfaring kommer
med milene!
    `,
    "mainPic": "./testname/img/en_img/57_b32d1c417bb64fe19723d3ddcbf8b6be.png"
  }
};
